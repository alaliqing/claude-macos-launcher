...
lines = [
    ...
    '  done',
    '  ',
    '  # Auto-type all @filenames (one keystroke per file to preserve encoding)',
    '  for item in "${ITEMS[@]}"; do',
    '    FILENAME=$(basename "$item")',
    '    # Check if filename needs quotes (contains spaces or special chars)',
    '    if [[ "$FILENAME" =~ [[:space:]] ]] || [[ ! "$FILENAME" =~ ^[a-zA-Z0-9._-]+$ ]]; then',
    '      # Type @"filename" with quotes for special characters/Chinese',
    '      osascript <<EOF',
    ...
    'EOF',
    '    else',
    '      # Simple filename without spaces, no quotes needed',
    '      osascript <<EOF',
    'tell application "System Events"',
    '  keystroke "@$FILENAME "',
    'end tell',
